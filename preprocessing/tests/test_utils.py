...
class TestBaseTextCategorizationDataset(unittest.TestCase):
    ...
    def test_get_index_to_label_map(self):
        ...
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base._get_num_samples = MagicMock(return_value=100)
        base._get_index_to_label_map = MagicMock(return_value={0: 'a', 1: 'b'})
        self.assertEqual(base._get_index_to_label_map(), {0: 'a', 1: 'b'})
...
class TestLocalTextCategorizationDataset(unittest.TestCase):
    ...
    def test_get_train_batch_raises_assertion_error(self):
        # Mocking a dataset with fewer samples than the batch size
        pd.read_csv = MagicMock(return_value=pd.DataFrame({
            'post_id': ['id_1', 'id_2'],
            'tag_name': ['tag_a', 'tag_b'],
            'tag_id': [1, 2],
        ...
        # Creating an instance with a batch size larger than the number of available training samples
        l = utils.LocalTextCategorizationDataset("fake_path", 3, min_samples_per_label=2)

        # Asserting that an AssertionError is raised when attempting to get a train batch
